...
class Sequencer extends Component {
  ...
  componentDidMount() {
    ...
    const interval = setInterval(() => {
      if (this.midiSounds) {
        this.setState({ timestamp: this.midiSounds.contextTime() });
      }
    }, 25);
  }
  onSelectDrumSnare(e) {
  ...
  render() {
    return (
      <div className="App">
        ...
        />
        {/* TODO: get the "loop" position, not the time since start of page load.. */}
        {/* <div>Position: {this.state.timestamp}</div> */}
      </div>
    );
  }
}
